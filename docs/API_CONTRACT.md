...
- Live shortlist candidates are post-validated for duration bucket compliance before returning; invalid bucket spans are filtered rather than padded. Responses include safe filter metadata: `eligible_duration_window_count`, `windows_after_quality_filter`, `excluded_window_reason_counts`, `demoted_window_reason_counts`, `average_window_quality_score`, `selected_window_quality_range`, `duration_window_count_sent_to_model`, `prompt_context_char_count_sent_to_model`, `returned_candidate_count`, `filtered_candidate_count`, `live_filter_reasons`, and `returned_duration_seconds_range`.
