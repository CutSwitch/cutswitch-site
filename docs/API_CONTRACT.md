...
- Live shortlist prompts use bounded, quality-scored duration-window input (`SOCIAL_REELS_LIVE_WINDOW_COUNT`, default `18`, bounded `6` to `24`) instead of sending the full app-scale transcript blob to OpenAI. Obvious intro/outro, setup, mic/audio/camera-check, product/promo, supplement or product ingredient, tasting/reaction, book/link/outro logistics, follow-up logistics, meta-editing, sponsor/ad, and filler windows are excluded or demoted when enough better windows exist.
- Live shortlist candidates are post-validated for duration bucket compliance before returning; invalid bucket spans are filtered rather than padded. Responses include safe filter metadata: `eligible_duration_window_count`, `windows_after_quality_filter`, `excluded_window_reason_counts`, `demoted_window_reason_counts`, `average_window_quality_score`, `selected_window_quality_range`, `selected_window_quality_distribution`, `selected_window_reason_counts`, `duration_window_count_sent_to_model`, `prompt_context_char_count_sent_to_model`, `returned_candidate_count`, `filtered_candidate_count`, `live_filter_reasons`, and `returned_duration_seconds_range`.
...
- The live shortlist prompt asks only for reduced-schema fields plus compact Smart Story Edit recipe metadata. Heavy enrichment fields such as `title_options` are reserved for a later full/enrichment pass.
